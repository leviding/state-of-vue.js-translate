...
代码库转移到 Vue 之前，他们的用户忍受着卡顿甚至是浏览器崩溃。太多的用户对他们的应用程序心有不满。他们的故事很好的证实了，Vue 可以有效构建安全、可靠、易维护切具有防御性的应用程序。
...
如果你正在寻找 Vue.js 的优秀企业级案例，那么 Behance 和 Adobe Portfolio 的案例就可以派上用场。他们的团队使用 Vue 零基础地建立了两个独立的产品，而且不仅于此。
...
在 Livestorm 案例中，联合创始人兼 CEO，Gilles Bertaux 描述了他们如何从零开始创造一个可盈利的产品。得益于 Vue 及其可复用的组件，他们的开发速度更快也更容易。
...
Jacob Schatz，GitLab的前端 Leader，解释了为什么他们决定从 jQuery 技术转移到 Vue.js，同时分享了他们遇到的主要挑战。他们对更好的UX（用户体验）的专注使得产品更理想，因此增加了销售量。
...
Chess.com 则不得不处理 Angular 1 项目中难以维护的遗留代码。他们发现，Vue.js使得15位远程开发人员的团队协作更容易。Chess.com 是一个服务全球1900万用户且拥有大规模基础设施的平台。在他们的案例中，你将了解 Vue.js 是如何化解了他们的难题。
...
最后一个案例与其他案例大有不同。Sylvain Simao，墨尔本Clemenger BBDO的技术主管，介绍了如何用 Vue.js 开发4到12周的短期项目。应对紧张的交付周期、大量的动画和特效需要实现、活动页面的高性能要求使他们面临的最大挑战。
...
Behance 是展示和发现创意作品类在线平台中的领导者。 
Adobe Portfolio 可以让打造自己专属创意作品展示网站的定制平台。
...
> 我们曾因为并没有太多大公司使用Vue而犹豫。但是，每当我遇问题（通常都是因为我的多虑），Vue都都可以很容易的解决，这让我感到惊喜。
>
> 来自：
...
Behance 是 Adobe 旗下的一家子公司，多年来他们一直在利用最新的技术和设计思想创造能够连接并帮助创意世界的革命性的产品。
...
> Yuriy解释说，在 Vue 之前，我们一直在使用自主研发的一个 MVC 框架，它依赖于 Hogan.js（mustache）和 jQuery。我们的框架无法以声明方式渲染，迫使我们手动同步数据到 DOM 上。它也无法将功能抽离成组件，控制单向数据流，也没有全面的文档。所以尽管已经使用了几年，我们还是决定转向一个可以让我们能够快速构建，减少出错，降低成本，快速上手的技术方案。
...
> Mustache 对我们特别重要，因为我们在后台和前端都使用了相同的模板（现在依然在用，在 behance.net 也是如此）。利用 Mustache 将首屏快速提供给浏览器对于我们和用户都是非常重要的。如果我们等待浏览器下载JS，解析，编译和执行它，然后才将页面显示给用户，实现与 Mustache 同样的速度是非常困难的。我们也特意寻找过具有服务器端渲染功能的框架。
...
我认为我们面临的最大挑战就是，由于我们决定不分裂我们的代码库并从一个新平台开始，我们不得不花费大量的时间抽离旧的代码来形成新的组件。Erin 补充说，将旧版代码重构到 Vue 的同时仍保持网站的传输功能和其余部分平衡运行绝对是一个挑战。
...
关于 Adobe Portfolio，我们从 nbd.js 开始，这是一个从我们原本已经不再维护的产品中提取出的 Backbone 的自定义版本，称为“在线操作”，我们用它来构建 Behance 网络的模块。Matt 补充说，它对反应式系统有限制，因此我们使用 Ractive 构建了“反应性”部分。
...
> 在我们切换到 Vue 之前，绝大多数的团队都在这里。一旦我们决定采用vue，我们需要一些小项目来练手。对我们来说，只需要非常小且只有前端功能且不公开访问的站点即可，就像我们的样式指南。这样，我们可以学习如何使用Vue，如何编写测试，并相对安全地对组件进行风格化。只有这样，我们才能安心投入更大的项目。这就是Behance Live，Yuriy回忆说。
...
> 在 Portfolio 项目中，我们的整个前端团队中总共有9个工程师采用 vue。我们的一些后端开发人员也开始学习vue。 Matt 解释说，Behance 产品中约有8位前端开发人员在使用 Vue 中进行开发。
...
> 两个团队确实有一些功能上的重叠（Adobe Portfolio和Behance）。Erin 补充说，我们在代码库之间共享了许多库和API，通常在两个站点之间会出现许多重叠的功能并需要特性展示。
...
> 对于比较大的应用程序，vuex 存储区也很难构造。我们决定使用命名空间模块。目前还不清楚每个路由/页面或数据类型（例如用户或项目）是否应该存在单个存储模块。创建特定的路由存储意味着跨路由的操作将不可重用。对我们来说，使它们具有数据特性是最好的解决方案，其中包含一个顶级路由存储模块，它结合了路由所需的模块。但是，这个解决方案还不够完美。Yuriy若有所思的说。
...
> 为了定义各种组件的角色，我们区分“页面”组件（路由器指向的第一个组件，也是与vuex交互的组件）和“哑”组件（仅将属性发送到子组件，将事件传输给父组件）。

Using Vue.js for almost a year now, Matt and his team managed to build
or rebuild a bunch of features.

使用 Vue.js 将近1年后，Matt 和他的团队终于构建或重构了一个功能分支。
...
> 我会说 Vue 超出了我们的期望。我们曾因为并没有太多大公司使用 Vue 而犹豫。但是，每当我遇问题（通常都是因为我的多虑），Vue 都都可以很容易的解决，这让我感到惊喜。她笑着说。
...
Yuriy 认为，Vue.js 提供给开发人员的可能性与其他框架一样多。然而，与一些框架相反，它使开发更容易...也更便宜。
